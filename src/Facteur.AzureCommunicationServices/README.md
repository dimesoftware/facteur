...
Lastly and obviously, there are the various mail services, also known as **endpoints** in Facteur. emails can be sent with good old SMTP, Microsoft Graph API, Plunk, Resend, SendGrid, Azure Communication Services, etc.
...
| Service                        | Command                                               |
| ------------------------------ | ----------------------------------------------------- |
| Azure Communication Services   | `dotnet add package Facteur.AzureCommunicationServices` |
...
Before using the Azure Communication Services endpoint, you need:

- An Azure account with an active subscription
- An Azure Email Communication Services resource with a provisioned domain
- A connection string from your resource (found in the **Keys** panel under **Settings**)
- A verified sender email address (MailFrom address from your domain)

For detailed setup instructions, refer to the [Azure Communication Services Email documentation](https://learn.microsoft.com/en-us/azure/communication-services/quickstarts/email/send-email).
...
public async Task SendConfirmationMail(string customerMail, string customerName)
{
  ...
  EmailRequest request = await composer
      .Subject("Hello world")
      .From("donotreply@xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx.azurecomm.net")
      .To("guy.gadbois@facteur.com")
      .Cc("jacques.clouseau@facteur.com")
      .Bcc("charles.dreyfus@facteur.com")
      .BuildAsync(new TestMailModel { Email = customerMail, Name = customerName });
...
For plain text emails, use the `AzureCommunicationServicesPlainTextMailer`:

``` csharp
public async Task SendPlainTextMail(string customerMail, string customerName)
{
  string connectionString = Environment.GetEnvironmentVariable("COMMUNICATION_SERVICES_CONNECTION_STRING");
  IMailer mailer = new AzureCommunicationServicesPlainTextMailer(connectionString);

  await mailer.SendMailAsync(async x => await x
      .Subject("Hello world")
      .From("donotreply@xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx.azurecomm.net")
      .To("guy.gadbois@facteur.com")
      .Cc("jacques.clouseau@facteur.com")
...
{
    x.WithMailer(y => new AzureCommunicationServicesMailer(connectionString, y.GetService<IEmailComposer>()))
...
- **Connection String**: Store your connection string securely. Use environment variables or Azure Key Vault for production environments.
- **Sender Address**: Must use a verified domain's MailFrom address from your Azure Communication Services resource.
- **Email Size**: Total request size including attachments is limited to 10 MB.
- **Rate Limits**: Sandbox environments have sending limits. Request increases for production use.
